...
# ARG_POSITIONAL_SINGLE([host],[Remote database host])
...
{
	local first_option all_short_options='upgdih'
	first_option="${1:0:1}"
...
print_help()
{
	...
	printf 'Usage: %s [-u|--user <arg>] [-p|--pass <arg>] [-g|--(no-)go] [-d|--(no-)skip-download] [-i|--(no-)skip-import] [--(no-)psql] [--(no-)maria] [--(no-)locale-fix] [--(no-)include-roles] [-h|--help] <host> [<dbname-1>] ... [<dbname-n>] ...\n' "$0"
